...
| `/api/me/theme` | PATCH | `{theme: "dark" \| "light"}` → writes `themePref`. **403 while impersonating — deliberately stricter than `PATCH /api/me`, do not "harmonise" it:** impersonation rewrites `session.user.sanityId` to the target, so without the guard a super-admin would persist their theme onto someone else's record from a UI action that looks local. A name correction is plausibly made on a member's behalf; a theme is not. `"system"` is rejected until Child F flips `enableSystem`. No `revalidate*` — per-member chrome, no ISR page renders it. |
