...
**Song-row `leadIds`.** Each row of `songs[]` may carry `leadIds`, 0–2 distinct canonical
`teamMembers` ids. The route refuses any row naming a leader — **400 `invalid_request`,
`details.issues: ["songs[i].leadIds"]`** — unless the target is a worship night
(`special_role.format === "worship_night"`) and every id is in that role's own `Lead`, as
loaded for this write. This check runs **after** the observed-target comparison above: a
Lead change that has already moved the role's `_rev` surfaces as the 409 instead, so the
admin always gets the same reload path rather than a 400 a retry cannot clear.
...
- **`GET /api/cron/flush-notifications`** — same secret-based auth (401 otherwise). Runs one
  ...
  every five minutes and is measurably starved by this repo's CI volume, ranging 0.09–2.03 runs/h (issue #25). See
