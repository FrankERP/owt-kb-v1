...
| `/admin` | `(client)/admin/page.tsx` | S | **Manager** | dynamic | Control Room; data fetched client-side from `/api/admin/*`. `?tab=` opens a specific tab, filtered by role. **No shell — the page is the workspace** ([ADR-0035](adr/0035-the-admin-shell-is-gone.md)): one tab bar, one keyed body, no panel boxes, and no page-level horizontal scroll. |
