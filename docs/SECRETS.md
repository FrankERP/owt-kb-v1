...
**So: treat Vercel and GitHub as write-only, and rotate rather than recover.** There is no source of truth to copy from once a value is set; the value's only home is the moment it was generated. **Cloud Scheduler is the exception, and that cuts the other way:** the job's header is readable by anyone with `cloudscheduler.jobs.get` on `eloquent-figure-421401` (a bare `describe`, or the console), so the production `CRON_SECRET` is as private as that project's IAM. Never copy it out of there; rotate.
...
3. Verify end to end, on both presenters:
   ...
   `status.code` is the `google.rpc.Code` of the last completed attempt: **empty is success (observed 2026-09-10 on the first live tick), `-1` is never attempted (observed the same day on the job before its first tick), `16` (UNAUTHENTICATED) is a 401** — the bearer no longer matches Vercel. The 401→16 mapping is from Cloud Scheduler's documented HTTP mapping, not yet observed on this job; record the first observed value here. `status` describes the last *completed* attempt and a sweep can run for 60 s, hence the 90 s wait — if `lastAttemptTime` has not advanced, wait and describe again.
...
- **Rotate:** `node scripts/dev-verify-rotate.mjs --apply --show` does the whole sequence in
  one command, with nothing to type — mints a new password, hashes it, patches the member,
  rewrites the three keys in `.env.local`, drops the stale storage state, and signs in to dev
  to prove it worked. Dry by default; `--apply` writes. The address defaults to an RFC 2606
  `.invalid` one, which no Google account can ever claim. The manual equivalent:
...
**Set since 2026-09-24** as part of the P0 release: Preview (~07:30) and Production (~10:56),
each generated separately by Frank. `vercel env ls` shows separate Preview and Production entries
(checked by name only, per the retrievability rule above — neither value was read back).
