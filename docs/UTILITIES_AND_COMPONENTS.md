...
| Component | Purpose |
|-----------|---------|
| `AdminPanel` | The Control Room's one tree: ONE `AdminRail` plus one body `key={tab}`/`animate-fade-in` (a tab change remounts the incoming panel). It owns the tab reducer, `?tab=` and the integrity hook — and NO panel's contents: Miembros left for `MembersPanel` in R5 Task 3, so nothing member-shaped (Fuse index, dialogs, member writes) is built on the five tabs that never show it. No `brand-surface` box around a panel and no `.brand-admin-shell` around the panel — the page is the workspace ([ADR-0035](adr/0035-the-admin-shell-is-gone.md)); `adminShell.test.tsx` is the guard. `?tab=` behaviour is unchanged (`adminTabUrl.test.tsx`, whose queries are scoped to the strip because both nav layouts are in the DOM). Calls `useIntegrityQueue()` once, at the top level, and feeds the queue to `IntegrityQueuePanel` and the tone/count to `AdminRail`. |
