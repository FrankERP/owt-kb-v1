...
| `PlannerGrid` | Renders the month grid `plannerModel` computes — dates across, seats down, every cell editable. Decides nothing; `MonthGenerator` owns `cells`/`counts` state and the Auto fetch. Owns the **three-column workspace**: Participaciones (216px, passed in as a node), the grid, and the candidate picker (240px, mounted only while a cell is active). The 216 is `CHART_COLUMN_WIDTH`, a content **floor** derived from `ParticipationSidebar`'s member row (212px), not a budget — it shipped at 190 once and the count column overlapped the bar by 12px on every row. **Pantalla completa** manages focus (in on enter, back to the ⛶ button on exit), traps Tab, locks body scroll, `inert`s the rest of `<body>`, and pads to `env(safe-area-inset-*)` because its only exit control would otherwise sit under the Dynamic Island. The grid scrolls horizontally rather than squeezing its `minmax(150px, 1fr)` date columns, and the row-label column is `position: sticky`. **Pantalla completa** portals the surface to `document.body` (a `position: fixed` child of `.brand-admin-shell` paints nothing in Safari), drops both side panels and switches the date track to `minmax(0, 1fr)` so a whole month fits without scrolling; Esc leaves it and nothing is lost but scroll offset. Its root carries `planner-wide`, which `app/brand.css` uses to lift the admin frame's 1280px cap through `:has()` — the only place that happens. |
...
| `ParticipationRail` | Placement only, and for the **Tablero alone** — puts `ParticipationSidebar` in the dialog's empty side gutter (`position: fixed`, 216px) from the Tablero (≥1380px) up, so the two-pane editor loses no width and gains no third scroll region. Below that it renders nothing. The gutter placement is `createPortal`-ed to `document.body` — a WebKit workaround, not a spec requirement: mounted under `.brand-facet-panel` (`relative` + `isolation: isolate` + `overflow: hidden`) it lays out and hit-tests correctly in Safari and paints nothing. Chromium paints it fine in place, so the portal looks removable and is not; see `ParticipationRail.tsx`'s header. Counts come from the caller (`boardParticipationRoles`): a rolling 56-day window anchored at the edited service's date, with the stored copy of that service swapped for the live seats. **The planner grid no longer uses this** — its chart is an in-flow left column of `PlannerGrid`'s three-column workspace at every width, because the admin page's 1280px cap left a 1512px laptop no gutter to put it in. |
