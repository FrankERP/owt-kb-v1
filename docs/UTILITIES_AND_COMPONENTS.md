...
- **`rootIndex`, `noteAt`, `semitonesBetween`, `transposeKey`, `transposeChord`,
  ...
  note doesn't get left behind. Both `transposeChord` and `transposeKey` return their input
  untouched at `semitones === 0`: `DISPLAY_NOTES` holds one enharmonic per pitch, so a round
  trip would re-spell a song written in `Db` as `C#` without anything having been transposed. `ChordChart` imports these rather than owning local copies; it
  no longer exports `transposeChord` itself. Tested in `app/utils/__tests__/transpose.test.ts`.
- **`tempoPeriodMs(bpm)`, `beatsPerBar(timeSig)`, `autoscrollPxPerSecond(heightPx, lines, bpm)`,
  ...
  beat, falling back to `DEFAULT_BPM` (80) on a falsy/non-positive bpm; `beatsPerBar` is the
  numerator of a written compás (`"6/8"` → 6) and falls back to 4 on anything it cannot parse,
  which is what tells the metronome where the accent goes; `autoscrollPxPerSecond`
...
- **`leadSeatIds`, `songItemLeadIds`, `validateSongLeads`, `carryOverSongLeads`,
  ...
  worship night's song, shared by the setlist writer, proposal approval, the setlist editor,
  the member cards and the outbox snapshot. `validateSongLeads` refuses the setlist PUT's
  `leadIds`; `carryOverSongLeads` filters (never refuses) a proposal approval's carried-over
...
- **`nextWeekends(todayIso, count)`**, **`weekendLabel(weekend)`** — **retired in F3
  (2026-09-12)**, deleted with `app/utils/weekends.ts` and `WeekendList.tsx`: the ten
  weekend pill rows they backed (R3, Fridays added F1) went with the pills. History
  only — the calendar (`AvailabilityGrid` on `/me/disponibilidad`) is now the one way
...
- **`MEMBER_AVAILABILITY_QUERY`, `MEMBER_PROFILE_QUERY`, `SERVICE_DATES_QUERY`, `horizon()`**
  ([queries.ts](../app/\(client\)/me/queries.ts)) — the three `/me` pages' shared reads (F3,
  2026-09-12). NEUTRAL (no `"use client"` and no import that pulls one in), so every Server
  Component under `app/(client)/me/**` may import AND CALL what is here (ADR-0028).
  `MEMBER_AVAILABILITY_QUERY` (`/me/disponibilidad`) carries `_rev` — LOAD-BEARING, the
  `ifRevisionId` precondition `PATCH /api/me/availability` requires. `MEMBER_PROFILE_QUERY`
  (`/me/ajustes`) carries no `_rev` — no profile write takes `ifRevisionId`, dropped in the
  Task 2 fix round. `SERVICE_DATES_QUERY` — every published service day in the horizon,
  deliberately not ministry-scoped: a kids volunteer marking a Saturday should still see the
  team has a service that day. `horizon()` — the CDMX `{ today, limit }` window (today → +365
  days) the calendar's twelve months need. One copy of each so a projection edit cannot drift
...
- **`LYRIC_EYEBROW`**, **`LYRIC_EYEBROW_BLOCK`**, **`dimRepeatMarkers(node)`**
  ([lyricMarkers.tsx](../app/utils/lyricMarkers.tsx), R6) — the lyric block's two typographic
  rules, as a NEUTRAL module (no `"use client"`, no hooks) so the song page, a Server
  Component, may call them directly (ADR-0028). `LYRIC_EYEBROW` restyles the section headings
  a sheet ALREADY has (and `ChordChart`'s `# ` labels) as rail eyebrows — it never ADDS a
  label, since decision N's budget would be spent here in one page. `LYRIC_EYEBROW_BLOCK` is
  that token plus `!mt-6 !mb-1`, carried by a bare `div`: a `p` loses the equally-`!important`
  tie to the prose wrapper's `prose-p:!mt-0` on Tailwind's emission order, and every section
  name ships flush. It carries **no `first:`** — `groupBySections` starts every group with its
  heading, so `:first-child` matched them all; the page's wrapper owns that with
  `[&>div:first-child>div:first-child]:!mt-0`, and the gallery fixture repeats the wrapper
  byte-identically. `dimRepeatMarkers` wraps `//` in a `text-ink-dim` span (full alpha — the
  `/70` modifier measures 3.60:1 in dark) with `aria-label="repetir"`, walking strings only.
  `lyricMarkers.test.tsx` guards the behaviour and all three source shapes.
...
- **`snapshot(dates, notes)`** ([useAvailability.ts](../app/components/availability/useAvailability.ts))
  ...
  the hook that owns the availability edits and the revision-guarded save, not in the view
  that draws them: `MyAvailabilityPanel` (on `/me/disponibilidad`, F3) calls `useAvailability`
  ONCE and hands that state down to `AvailabilityGrid`, which only renders — a second hook call
  would be a second revision racing the first into the same document. The hook's
  ...
  the old ten weekend rows could not express. It skips days before `todayIso` at either end,
  caps the span at 366 days, and no-ops on a reversed range; its one caller since F3 is the
  grid's drag-select release (the Desde/Hasta `DateField`s F2 added were retired with the grid's
  «Rango por fechas» disclosure).
...
- **`popoverPosition(rect, viewportW, viewportH)`**
  ...
  fires scroll and resize. `y` is clamped to a minimum of 8px so flipping above a day near
  the top of a short viewport — including a drag-select range (F2) anchored to the first
  mounted day after a page flip — cannot push the popover negative.
...
| Component | Purpose |
|-----------|---------|
| `SongSheet` [C] | Full lyrics + chords overlay (PortableText, focus-trapped, play-history) — driven by `PlayerContext`. Its meta row's BPM is a real `TempoPill` (`size="sm"`, `enabled={isOpen}`) when `bpm` parses to a positive number, and the old static span when it does not ("libre", "70-80"); the row's static chips carry `min-h-[44px]` so they match the pill's tap-target height. |
...
| `TransposeProvider` / `useTransposeOptional` [C] | `app/components/song/` — the ONE transposition seat for a song page: `nativeKey`, `semitones` (always wrapped into 0..11 by `setSemitones`), `soundingKey` (null when the song has no key). `useTransposeOptional()` returns null outside a provider, which is how `ChordChart` falls back to its own state on the practice sheet and the admin preview. The song page seats `nativeKey` on the key of the FIRST chart when that chart is ChordPro (`post.chords[0]`, which is the one `ChordChart` opens on), falling back to `post.key` — a chart may be written in another key than the song, and the dial, the drawer's «(original)» hint and the chart's own readout all have to name the same one. Tested. |
| `SongAudioSection` [C] | A song's audio tracks, wired to the player. |
| `SongHeroPills` [C] | `app/components/song/` — the song hero's key / BPM / compás row AND the transposer drawer under it. The key is a `KeyDial` only when the drawer has something to move — `transposable` (the song carries a ChordPro chart) or `mixTones` with more than one key (rehearsal mixes the `RehearsalPlayer` will switch between) — and the written key parses; otherwise it stays the static `brand-key-dial` badge, because a drawer with nothing to move is a control that lies. A key that has a mix carries a dot under the note and «con mix de ensayo» in its accessible name. The drawer is a `Collapse` holding the ONE 12-key `SegmentedControl` — `ChordChart`'s ± pair steps the same provider value, so hero and chart can never disagree. Each option's visible label is wrapped in `normal-case` — the control uppercases its options for CALENDARIO / LISTA, and a flat left alone reads "EB". A BPM the page could not parse into a number arrives as `bpmText` and renders as the static pill it always was: a tapable tempo needs a number, legacy text still deserves to be shown. Tested. |
| `KeyDial` [C] | `app/components/song/` — the hero key as a disclosure button. Same `brand-key-dial` chrome as the static badge (R4 ruling 8: a transposable song does not get a different-looking key, it gets one you can press), a `NumberRoll` readout of the SOUNDING key from `useTransposeOptional()` and a chevron that rotates when open. `keyLabel` is what it reads when there is no provider value to read — `SongHeroPills` passes the key the page seated the provider on (the first chart's key on a transposable song), so the dial never paints a different key from the chart. Tested. |
| `TempoPill` [C] | `app/components/song/` — the BPM pill, tapped to beat AND click the song's tempo (`{ bpm, timeSig?, enabled?, size? }`). The ONE tempo control on two surfaces (F2, ruling 14): the song hero (`size="md"`, the console pill) and `SongSheet`'s meta row (`size="sm"`, the outlined pill that row already drew as static text) — same ring, same click, same 44px target. TWO CLOCKS (F1, ruling 11): the RING is CSS and has no timer (R4 ruling 7) — `.brand-tempo-pill::after` in `brand.css` scales and fades once per beat at the pill's own `--tempo-period` (`tempoPeriodMs(bpm)`), so nothing drifts and the global reduced-motion rule collapses it with everything else — while the CLICK is a `createMetronome` (`metronome.ts`) built on the FIRST tap, from `bpm` + `beatsPerBar(timeSig)`, and stopped on the second tap, on a hidden tab (through its `onStop`), on unmount and whenever `bpm`/`timeSig` change underneath a running beat (the metronome is built from the props of the tap, so a changed tempo drops it rather than beating the old one). `enabled={false}` (F2, ruling 16 — `SongSheet` passes `enabled={isOpen}`) stops it and drops the pressed state without waiting for an unmount, for a surface that is dismissed but kept mounted, and the handler takes no tap while it is false (never `disabled`, so the chrome and focus behaviour are unchanged). ONE metronome sounds app-wide (ruling 15): a second pill's tap takes the floor and this one un-presses through its `onStop`. Tap = ring + click, no separate silent mode (ruling 13); on iOS the click obeys the silent switch, because Web Audio does. `data-active` is absent while idle. Tested — including that the timer count is zero before the first tap, after the second, after a dismissal and after unmount, and that each size renders its own chrome. |
| `metronome.ts` (`createMetronome`, `CLICK`) | `app/components/song/` — the click under the tempo ring (R4 ruling 11), `TempoPill`'s only import. Browser-only but React-free and `motion`-free. A "tale of two clocks" LOOKAHEAD SCHEDULER: a self-rescheduling `setTimeout` every `CLICK.lookaheadMs` (25 ms) books every beat falling inside the next `CLICK.scheduleAheadMs` (100 ms) on `ctx.currentTime`, each one an `OscillatorNode` (sine; `accentHz` 1000 on beat 1 of the bar, `beatHz` 800 otherwise) through a `GainNode` ramped to silence over `CLICK.lengthMs`. Never a `setInterval` — a stop must never race a queued tick. A tick that ran LATE skips the beats already behind the clock and keeps the bar's accent phase, because `osc.start(pastTime)` fires immediately and a stall would otherwise flam every missed click at once. `stop()` clears the loop, stops every oscillator it had already booked BY HAND — suspending only freezes the clock those clicks are pinned to, so they would sound off-phase on the next resume — and then suspends the context, never closes it (a closed context cannot be resumed). The booked list is pruned on every tick, so a rehearsal-long run holds at most one scheduling window of nodes. The `AudioContext` is created LAZILY on the first `start()`, which therefore has to be called from a user gesture, and is reused across starts; where none exists (prefixed or not) `start()` is a silent no-op and the pill still rings. A `visibilitychange` to hidden stops and reports through `onStop`. ONE metronome sounds at a time, APP-WIDE (F2, ruling 15): the module holds a "current" and a `start()` takes the floor from whoever had it, stopping that instance and reporting through ITS `onStop` so its pill un-presses instead of ringing over silence — two pills can be on screen together (the song hero under a `SongSheet` opened over it) and two tempos at once is noise. A pill that cannot sound at all (no Web Audio) never silences the one that can, and a late `stop()` from the loser's own cleanup does not release the new holder's floor. `stopCurrentMetronome()` silences whatever is sounding from outside any instance. Tested in `app/components/song/__tests__/metronome.test.ts` — including the stalled-clock catch-up, the hand-stop and the hand-over. |
| `LyricsAutoscroll` [C] | `app/components/song/` — the «Autoscroll» / «Detener» pill on the Letra section header (R4 ruling 6): hands-free lyrics at the song's own tempo. Drives the WINDOW (`window.scrollTo(0, y)` per frame), never a transform on the section — a transformed ancestor becomes the containing block for every `position: fixed` descendant (ADR-0031), and the audio transport, toasts and song FAB all live on this route. `requestAnimationFrame` only, never `setInterval`: `dt` is the gap between two real frame timestamps, clamped to 50 ms so a backgrounded tab cannot leap. Speed is `autoscrollPxPerSecond(el.offsetHeight, lines, bpm)` measured once at start (`bpm: null` → `DEFAULT_BPM`); `lines` is the ChordPro chart's `countLyricLines` or, for PortableText, one block ≈ one line (an approximation the clamped speed absorbs). Stops itself when the section's `bottom <= window.innerHeight - bottomInset`, where `bottomInset` is the phone tab bar's published `--bottom-nav-h` plus the audio transport's measured height (`.audio-player`) — the last line has to clear the furniture over the bottom of the viewport, not the viewport's edge. Re-measured at every resume, since the transport appears the moment someone taps play. A `touchstart` / non-mouse `pointerdown` pauses; `touchend`, `touchcancel` and `pointerup` lift it (a promoted drag fires `touchcancel`, never `touchend` — miss it and the pill sits on «Detener» for good). **Never `pointercancel`:** Chrome Android and iOS Safari fire it the moment a touch becomes a pan, with the finger still down, so binding it as a lift resumes the loop under the drag and fights the momentum; a genuinely cancelled touch reports `touchcancel` anyway. A lift that actually scrolled waits for `scrollend` rather than resuming into iOS momentum, arming a 400 ms (`SCROLLEND_FALLBACK_MS`) `setTimeout` backstop that resumes on its own if `scrollend` never arrives — cleared on `scrollend`, toggle-off, unmount, at the TOP of every lift (one gesture lifts twice, `pointerup` then `touchend`, and re-arming over the handle would orphan the first timer into a resume with no listeners left) and on the next `down()` — a finger back on the glass inside the 400 ms window must beat the timer, which `resume()` also enforces by returning early while `touching`; a lift that scrolled nothing resumes at once. Resume re-seeds `y` from `window.scrollY` and keeps re-seeding for the next 3 frames, but never every frame — at low speeds a sub-pixel advance rounds back to the same scroll offset and the loop would stall. A `wheel` ends the run outright, because a mouse has no lift event to resume on and a control reading «Detener» over a still page lies (scrollbar and keyboard scrolls are not intercepted — phone-first by design). The pill carries `min-h-[44px]`. Every listener is `{ passive: true }`; the teardown cancels the frame, clears the timer and sets `finished`, so no late callback can resume a run that is over. **Not offered at all when the section already fits the viewport (F3):** on mount and on `resize` it compares `el.offsetHeight` against `window.innerHeight` minus the same bottom inset and the section's resolved `scroll-margin-top`, and renders `null` when there is nothing to scroll — the run's end check is true on its first frame for a short lyric, so the pill used to toggle itself off the instant it was tapped. Tested. |
...
| `TutorialPoster` [C] | `app/components/song/` — **a tutorial embed that costs nothing until it is asked for (R6, spec §19.5 row "Tutorial embeds", decision P)**. Three YouTube iframes used to boot on every song-page load; each one now paints YouTube's own still (`i.ytimg.com/vi/<id>/hqdefault.jpg` through `next/image`, allow-listed in `next.config.mjs`) under a centred `Button variant="primary" size="lg"` «Reproducir», and mounts the player inside a `Presence variant="fade"` on press. The id comes from `extractYouTubeId`; a url it cannot parse (a Vimeo link, a bare embed URL) renders today's raw iframe unchanged — the poster is an enhancement for YouTube, never a gate. `url` is OPTIONAL because the Sanity `tutorial` object requires neither field, and a row with no url renders nothing rather than an iframe with no `src`. The image's `alt` is empty: the button carries the accessible name. |

### Rehearsal (`app/components/song/`, pure logic in `app/utils/rehearsalMixes.ts`)
| Component | Purpose |
|-----------|---------|
| `rehearsalMixes.ts` | `app/utils/` — the rehearsal player's pure half, neutral (no `"use client"`, no client imports) so the song page, a Server Component, may call `preselectMix` (ADR-0028). `groupMixes(mixes)` buckets `full` mixes first, then `up` mixes by `family` in `FAMILY_ORDER` (bass, keys, organ, synth, drums, electric, acoustic), each group sorted by `track` with an `es` collator. `preselectMix(mixes, instruments?)` — the mix highlighted on arrival: the first track of the family the member's FIRST declared instrument maps to (`SEAT_TO_FAMILY`), else the full mix, else the first mix; highlight only, nothing autoplays. `waveformBars(peaks, bars)` collapses the 600-value 0–255 envelope to `bars` values in 0..1 (max per slice) for the canvas. `isActiveAt(active, seconds)` reads the `[[s,e]…]` spans. `mixTones(mixes)` — the keys a song has mixes in, one spelling per pitch, in pitch order. `mixesForKey(mixes, key)` → `{ tone, exact, mixes }`: the rows in that key (enharmonic, via `rootIndex`), else the NEAREST key's with `exact: false` (a tie goes to the lower key); a null key or no parseable tone shows everything. |
| `Waveform` [C] | `app/components/song/Waveform.tsx` — one `<canvas>` per row: the isolated instrument's 600-peak envelope as bars, `active` spans painted in `--accent-rgb`, the rest in `--mono-400-rgb`, brighter once played, plus a playhead once `progress > 0`. Colours come from `themeColour` (a canvas can't read Tailwind classes). Redraws via a `ResizeObserver` created once ([] deps) that calls through a ref holding the latest `draw`, so a resize is never missed while progress ticks don't tear it down. With `onSeek` it renders as a `<button>` (click = fraction of width, ←/→ = 5%) and the canvas is `aria-hidden`; without `onSeek` the canvas alone carries `role="img"`/`aria-label` — never both, which would give assistive tech two nodes announcing the same label. |
| `RehearsalPlayer` [C] | `app/components/song/RehearsalPlayer.tsx` — `{ mixes, songId, songTitle, songSlug, preselect? }`, presentational so the song page (Server Component) can render it and the theme-gallery fixture can host it with inline data. **One `<audio>` rule:** it drives the app's single player through `PlayerContext` (`usePlayer`), never a second element — the fixed `AudioTransport` keeps working unchanged, and switching mixes reads the current position before the src changes and restores it once the new file's `loadedmetadata` fires (a `pendingRestore` ref cancels a still-in-flight restore from an earlier switch, or a third switch's listener would apply the stale second value). **URLs are always `/api/audio/[songId]/[key]`, never `cdn.sanity.io`** — `urlFor(m)` is the only place a track URL is built, so the session gate on that route is the only path to the file. **The key is the hero dial's** (`useTransposeOptional` — `SongHeroPills` is the one picker, so the player has no strip): rows are `mixesForKey(mixes, soundingKey)`, a caption above them says which keys exist and, when the sounding key has none, which key stands in; turning the dial while a row PLAYS re-plays the same track in the new key at the same position, a paused row is left alone; without a provider every key's rows show. Groups render via `groupMixes`; each `up` mix with `peaks` gets a `Waveform`; download is a plain `<a href="{urlFor}?download=1" download>` — best-effort across the cross-origin CDN redirect, with the route's own `?dl=` doing the real work. |
...
| Component | Purpose |
|-----------|---------|
| `DayCard` [C] | **The core service card / R1 run-sheet card** — day · date header, countdown pill, setlist (medley-grouped via `buildRuns`) + all five seats; embeds `SetlistEditor` for admins + `PracticePlaylistButton`. `layout?: "card" \| "wide"` — `wide` puts setlist and team side by side from `lg` (only when both a setlist AND a team are present); `hero?: boolean` makes the card the page's one primary action, `PracticePlaylistButton variant="hero"` ("Ensayar") in the header, dropping the inline pill from the Setlist rail so the affordance never repeats. Its ⚠ duplicate-seat marks read `findDuplicates` from `app/utils/agenda.ts` (R2 — moved out of this file so the schedule agenda's conflict count can never disagree with the card's own marks). **F3:** a long press on a setlist row reports it through `onQuickActions(song)` — the ROW opens nothing; the CARD owns ONE `QuickActions` (`open={actionsOpen}`), the `LibraryIndex` rule, because a mounted sheet subscribes to the CueDialog layer context. Two actions, the library's: «Abrir» (`openSheet(_id, play_key)`, what the tap already does) and «Copiar enlace» (`/posts/<slug>`, confirmed or refused by a `useToast` toast, offered only when the row carries a slug). Rows carry `select-none` — before this a hold selected the row's text on iOS. `DayCard` therefore needs `ToastProvider`, which `app/utils/Provider.tsx` mounts app-wide. |
| `DayCardDisclosure` [C] | A non-next service on `/` or `/me`, collapsed to one line (day · date · countdown) that opens a `Collapse` onto its `DayCard`. **R7:** a long press opens its `QuickActions` — «Ver en calendario» (`router.push("/schedule")`) and «Añadir a mi calendario» (this ONE service as an `.ics` through `buildICS`, the same blob download `AddToCalendarButton` does for the member's whole list). The UID is `serviceId ?? roleId ?? normalizeText(`${date}-${day}`)` — the Sanity id goes in RAW (a `_id` may carry uppercase, and `/me` exports it untouched, so folding it here would mint a SECOND UID for the same service); only the day/date fallback is `normalizeText`ed, because `day` is a Spanish label. `serviceId` is the service document's own `_id`, set at both `/` and `/me` call sites, so a weekend service gets the SAME UID `/me`'s own export uses (`<_id>@owt`). |
...
| `availability/MyAvailabilityPanel` [C] | **`/me/disponibilidad`'s only body, alone since F3 (2026-09-12)** — renamed off `AvailabilityPanel` in fix round 1, which shared its name with the unrelated admin `AvailabilityPanel` below (different directory, different job): calls `useAvailability` once and renders the shared «Repetir…» recurring panel, «Guardar» (`busy`/`busyLabel`; success is a `useToast` "Guardado ✓", not an inline flash), the dirty/error/held-conflict notices, `AvailabilityGrid` — now mounted OPEN, no disclosure — and the one `NotePopover` it opens. **F3 retired `WeekendList`** (the ten weekend pill rows) and the panel's «Ver calendario» `Collapse` with it: the grid is the one surface, always visible, not a fallback behind a toggle. `openNote(iso, anchor, isos?)` forwards an optional `isos` (a dragged range, F2) into the `NoteAnchor`. «Quitar serie» calls `closeNote()` before dropping its dates, so a popover pinned to one of them cannot outlive it. |
| `WeekendList` | **Retired in F3 (2026-09-12), file deleted.** `/me`'s original default availability surface (R3; Fridays added F1) — the next ten weekends as `VIE`/`SÁB`/`DOM` pill toggles. Frank's ask ("too many buttons that select the unavailable dates … remove the pills with the calendar") replaced it with the grid alone; `tone="availability"` on `Button` survives only for `AvailabilityGrid`'s «Seleccionar fechas» pill. |
| `AvailabilityGrid` [C] | **The twelve-month grid — the ONE way to mark availability since F3 (2026-09-12)**, moved to `app/components/availability/AvailabilityGrid.tsx` in fix round 1 (was `app/components/AvailabilityCalendar.tsx`) — a controlled view (`state`, `serviceDates`, `openNote`, `closeNote`, `noteIso`), mounted open on `/me/disponibilidad` with no disclosure around it (F3 retired the panel's «Ver calendario» `Collapse` along with `WeekendList`). Owns paging AND the F2 drag-select gesture: a «Seleccionar fechas» ⇄ «Listo» `Button` (never a long-press — that gesture is already the phone browser's text-selection/callout trigger) arms pointer-capture drag over the months container (`touch-action: none`, `user-select`/`-webkit-user-select: none`, `-webkit-touch-callout: none`, `onContextMenu` cancelled while armed); `isoFromPoint` (`dragSelect.ts`) resolves the day under a captured pointer via `elementFromPoint`; one pointer (`isPrimary`/`pointerId`) owns the gesture and a `dragRef` mirror lets `pointerup` read the live span rather than a possibly-stale render closure. The next month fades in as an absolutely-positioned OVERLAY inside the last visible tile's own bounds (`shadowOpacity` over a 120px reach from the tile's bottom) — never a fourth tile that would grow the page — `pointer-events-none`/`aria-hidden` while fading, and solidity LATCHES for the rest of the drag once the finger first reaches the tile's bottom edge (the overlay's own cells sit inside the host's padding/border and never reach that edge themselves). Release calls `applyRange`, flips the page if the end landed in the shadow month, and opens the range's `NotePopover` from a post-commit effect anchored to the first still-mounted day (`scrollIntoView({ block: "nearest" })`). **F3 retired the Desde/Hasta `DateField`s under «Rango por fechas»** — four ways to express one range (grid tap, «Repetir…», drag, Desde/Hasta) was the accretion Frank asked to undo; a range is a drag, a single day is a per-day tap, a pattern is «Repetir…» on the host. **Read-only outside «Seleccionar fechas» (Frank's look, 2026-09-13):** `handleDateClick` is gone — every day cell carries no `onClick`, `aria-disabled="true"` and `tabIndex={-1}` when the mode is off, so a casual look at the calendar can no longer mark or open a day by accident; inside the mode a tap is a one-day drag through the same `onPointerDown`/`onPointerUp` flow as a real range, which already marks the day and opens its note. |
| `NotePopover` [C] | **The «Razón» note editor (R3; range-aware F2)** — one non-modal positioned field per panel (never a `CueDialog`: nothing underneath goes inert), owned by `MyAvailabilityPanel` so `useAvailability`'s `onAdopt` can close it in the same update a conflict adopts the server's dates. `NoteAnchor` gained `isos?: string[]` (F2) — the whole dragged range, ascending; when present, one «Razón» edits every day in `isos` (`setNote` called per day) and «Quitar estas fechas» removes every day, with the title from `dragSelect.ts`'s `rangeLabel`. `iso` stays the anchor/first day and the popover's positioning key either way. Exports `popoverPosition` and re-exports `fmtDayLabel` (moved to `dragSelect.ts` in F2 to avoid an import cycle — `rangeLabel` needs it and `NotePopover` needs `rangeLabel`). **Portalled to `document.body` (Frank's look, 2026-09-13):** it renders `position: fixed`, and on `/me/disponibilidad` it used to mount inside the route-reveal host (`[data-reveal]`, an animated `transform`), which is the containing block for a `fixed` descendant — reproduced on dev at 1440×900, day cell at ~(503,615), popover rendering at ~(785,845). `createPortal` to `document.body` (the `Toast.tsx`/`CueDialog.tsx` pattern, SSR-safe `useState` set in an effect) escapes it. |
| `availability/dragSelect.ts` | **The F2 drag-select model** — pure and neutral (no hooks, no DOM writes) because jsdom performs no layout, so the arithmetic is tested here rather than through a rendered grid: `isoRange(a, b, todayIso)` (every day inclusive, either order, days before `todayIso` dropped, capped at 366), `shadowOpacity(distancePx, reach = 120)` (0 at/beyond `reach`, 1 at 0, linear between, `NaN`/negative → 1), `isoFromPoint(x, y, doc?)` (`elementFromPoint(x,y)?.closest("[data-iso]")`, refusing a shadow-month host whose `data-shadow="true"` ancestor is still `data-solid="false"`), `rangeLabel(first, last)` (the popover's Spanish range title), and `fmtDayLabel` (moved here from `NotePopover.tsx`, re-exported from there for existing importers). |
...
| `Navbar` [S] | Top navbar shell; deliberately **non-async** (session resolved client-side) so pages stay ISR-renderable. Props: `title`, `author?`, `cue?` — the `tags`/`schedule` flags were removed in R5 (ruling 9). |
...
| `BottomNav` [C] | Mobile bottom tab bar — three worship tabs (Inicio · Calendario · Biblioteca) or the kids set; «Más» only when Kids / Planear Kids / Admin apply; hidden when fewer than two items; «Más» is a `CueDialog` sheet. Publishes its measured height as `--bottom-nav-h` + a `has-bottom-nav` class on `<html>` while on screen; hidden ≥ `lg` and on `/auth*`/`/studio*`. **Since R6 it is the STATEFUL half only** — the session, the pathname, the tab/row derivation and the measurement effect — and it renders `BottomNavBar` for everything that paints, forwarding its `barRef`. |
| `BottomNavBar` [C] | **The tab bar's presentational half (R6 Task 7)** — `app/components/BottomNavBar.tsx`, props only: `tabs`, `activeHref`, `moreRows`, `moreOpen`, `onMore`, `onNavigate?`, `barRef?` (it also exports the tab glyphs). Draws the `<nav>`, the `SlidingIndicator` and the «Más» `CueDialog` sheet; the rendered DOM is exactly what `BottomNav` drew before the split. The split exists so the theme gallery's `nav` fixture can host the bar — a public, prerendered route cannot call `useSession` (ADR-0017) — and it is the general shape: **a gallery fixture hosts the presentational half, never the component that reads a session.** `bottomNavOffsetSync.test.ts` and `textZoomLayout.test.ts` read this file now. |
| `NavLinks` [C] | Desktop nav link row, rendered in the navbar's centred title block at `lg`+ (the title itself goes `lg:hidden` there); one shared `SlidingIndicator` underline. **Takes no props:** ministry membership alone gates the links, so Calendario and Biblioteca show for every worship member on every page — the old `schedule`/`tags` flags were passed inconsistently and dropped both links on `/kids`. |
| `SectionNav` [C] | In-page section anchors, plus — since R4 — the song page's practice cluster. One `IntersectionObserver` on `#song-hero` (threshold 0) flips `heroGone`, inset by a `rootMargin` of `-<sticky top + bar height>px 0px 0px 0px` — the bar's STUCK position, read at observer creation from `getComputedStyle(bar).top` (which resolves the navbar height plus the safe-area inset) plus its own rect height. **Never the rect's `bottom`:** at mount the page is at scrollY 0 and the bar is still in flow BELOW the hero, so that bottom is most of the hero's height and the shrunken root would latch `heroGone` true from load — the cluster would show over a hero still on screen and never hide — otherwise the hand-off waits for the viewport's top edge and the hero's controls are already hidden behind the chrome while the bar still shows nothing, and the optional `practice` prop then fades in through a `Presence variant="fade"`. The right-hand slot (`ml-auto … shrink-0 … min-h-[44px]`) is ALWAYS rendered, empty or not, so the bar's height never changes when the cluster arrives; the links keep their own scroll box and scroll behind it on a phone. |
| `PracticeCluster` [C] | `app/components/song/` — what the hero was carrying, once the hero is gone (R4 ruling 1): the title (desktop only, `hidden lg:block`), the SOUNDING key from `useTransposeOptional()` in `brand-key-dial` chrome with a `NumberRoll`, the BPM, and one 44 px play/pause `Button variant="icon" size="lg"` over `usePlayer()` (`playTrack` when the track is not the loaded one, `togglePlay` when it is). It rides the song page's own sticky section bar, NOT the navbar — the navbar centre is already the song title on phones and the nav links at `lg`. Tested (`sectionNavPractice.test.tsx`). |
...
| `Button` [N] | The house button: six variants, three sizes, press physics, primary sheen, `busy`, `href`. The `pill` variant takes a `tone` (`"accent"` default, `"availability"` — `AvailabilityGrid`'s «Seleccionar fechas» pill since F3 retired its original consumer, `WeekendList`'s weekend toggles; pressed text is `soft`, not `strong`, to clear 4.5:1 in light). The `icon` variant takes the same prop with one value, `"danger"` — the destructive hover for a row action (`hover:text-negative-fg hover:bg-negative-surface/40`, Contenido's «Eliminar»); it is in the primitive, never in a caller's `className`, or two `hover:` pairs race on one element. |
...
| `CueDialog` [C] | The ONE dialog shell — never a hand-rolled `fixed inset-0` scrim. `mode="modal"` \| `"sheet"` (a sheet is only a sheet below 640px); drag-to-dismiss from the sheet's head (handle + title bar — on a phone it is the only visible close control, the × is `sr-only` there; 150 px of travel or a 0.5 px/ms flick — `SHEET_DISMISS`); `onDismiss(reason)` where `reason` is `"escape" \| "backdrop" \| "drag"`. Traps focus, stacks layers, supports a portalled focus "satellite". **Its focus effects depend on the presence edge only** — entry focus is `[open, top]` and skips when focus is already inside the shell; layer registration is `[id, mounted, registerLayer]`. Adding `onDismiss` (an inline arrow in most consumers, and guaranteed stable in none) back to either one re-runs it on every keystroke and throws the caret onto the close button — the bug that stopped members editing their profile ([ADR-0034](adr/0034-dialog-focus-effects-depend-on-edges-only.md)). Always render `<CueDialog open={x}>`, never `{x && <CueDialog open>}` — see `cueDialogMount.test.ts`. |
...
| `Menu` / `MenuItem` / `MenuSeparator` / `MenuHeader` [C] | The ONE anchored dropdown. `role="menu"`, roving focus, merges the trigger's own ref. Inside a `CueDialog` the menu owns Escape (first Escape closes the menu, the second the dialog) — M0b-2. **The panel is PORTALLED to `document.body`** and positioned `fixed` from the trigger's `getBoundingClientRect()` when it opens (R5 Task 7 fix round 1): `absolute` inside the wrapper was clipped by any scrolling or `overflow-hidden` ancestor — `/biblioteca` → Filtros → Tonalidad showed two of ten rows inside `CueDialog`'s `overflow-y-auto` body — and a `fixed` fallback would still be trapped by the transformed route-reveal host, so a portal is the house answer (`Toast`, `NotePopover`). It flips above the trigger when there is no room below, repositions on `resize`, closes on any ancestor `scroll` (capture phase; the panel's own scroll is exempt), sits at `z-[92]` (above a dialog's `z-[90]`, below the toast stack's `z-[95]`), carries `data-pull-ignore`, and its outside-click test treats the panel as inside even though it is no longer a DOM descendant. Fix round 2: Tab from an item closes with `close(true)` — the panel is the last child of `<body>` and every item is `tabIndex={-1}`, so closing without refocusing dropped focus out of the document; the panel is sized to the room it HAS (inline `maxHeight`, the CSS `max-h` staying the cap) because the 240 px flip threshold is below the 320 px height and a panel could hang off the bottom edge; item focus is `preventScroll` + `scrollIntoView({ block: "nearest" })`, so moving focus can never scroll the document into closing the menu; measurement reads `documentElement.clientWidth/clientHeight` (not `innerWidth/innerHeight`, which include a classic scrollbar's gutter), with a window fallback for jsdom; a start-aligned panel near the right edge is clamped back after its width is known; and the surface is the opaque `bg-surface-raised` plus `backdrop-blur-sm` — over a dialog the previous one let the text underneath read through. `CueDialog`'s Escape yield still matches it (`event.target.closest('[role="menu"]')`). The panel is `max-h-[min(20rem,60vh)] overflow-y-auto` — a 30-option `Select` scrolls instead of running off the viewport. `className` sets the root wrapper's classes on top of `relative` (default `inline-block`; `Select`'s desktop trigger passes `block w-full`). `MenuItem selected` marks the current row (`aria-current="true"` + the accent tone) and takes keyboard entry focus ahead of the first item. |
...
| `SlidingIndicator` / `useActiveIntoView` [C] | The active marker for tab bars (`AdminRail`, `SectionNav`, `BottomNavBar`). Semantics stay on the items (`aria-current`); the hook scrolls the active item into view. |
| `Switch` [C] | The ONE switch — `role="switch"`, `aria-checked`, a `<button>`; knob springs with `initial={false}`; haptic on flip. Sizes `sm`/`md`. |
| `Checkbox` [N] | The ONE checkbox — native input stays (`sr-only peer`) and does the work; the box is drawn, the mark scales in. `tone="negative"` is the destructive tone (the theme gallery demonstrates it; the kill switch, its former consumer, is a menu item behind a confirm since R5 Task 3). `align?: "center" \| "start"` (default `center`; `start` for a two-line label) is a prop, not a `className` — a same-property utility passed through `className` cannot beat one the primitive already sets, since stylesheet order (not specificity) decides which of two same-specificity classes wins. |
| `Select` [C] | The ONE select — two renders of one control (R5 ruling 8). Touch keeps the native `<select>` under tokenised chrome plus a drawn chevron; on a fine pointer (`(hover: hover) and (pointer: fine)`) the native element goes `sr-only` — still the form value, still labelled, still tab-reachable — behind a `Button variant="secondary"` trigger that opens a `Menu` of the `<option>`s, with a first-letter jump (bound on the control's own wrapper, so it also works from the trigger after a click-open). Detection is `useSyncExternalStore` over the media query (server snapshot `false`), so SSR/jsdom stay native and a mouse attached mid-session switches without a remount. Choosing sets the native `value` and dispatches a real bubbling `change`, so every consumer's `onChange`/`e.target.value` is untouched — except when the picked option is ALREADY the value, which dispatches nothing (a native select fires no `change` there, and the spurious one sent a `PairRoster` PATCH). Fix round 1 also: `Menu className="block w-full"` + a `truncate` label span, so a capped wrapper (`MonthGenerator`'s `w-24 max-w-[96px]`) truncates instead of spilling; `tabIndex={-1}` on the `sr-only` native element, which was an invisible tab stop in front of the trigger (it keeps `htmlFor`, so the `<label>` and AT still name it); `MenuItem selected` on the current option (`aria-current`, and keyboard entry focus lands there); and a controlled value matching NO option leaves the trigger blank rather than naming the first. `popover={false}` forces native. Sizes `sm`/`md`/`lg` (`lg` = `md` plus a 44 px minimum height on the element itself). `label` + `id`, or `aria-label`. |
| `DateField` [N] | The ONE date/month input — native under tokenised chrome; `kind="month"` with `onStep` draws the prev/next month buttons. **`disabled` reaches the arrows too (R6 Task 1 fix round):** a composite field is ONE control, and the Kids planner's month arrows stayed live mid-generate and mid-save, so a press started a second month load underneath the first. |
...
| `useLongPress(onLongPress, { ms?, move? })` [C] | The ONE long-press gesture (R7, spec §12.8) — 450 ms of a still primary pointer, cancelled by a lift, a cancel, the pointer leaving the row, a move past 8 px, or ANY `scroll` (capture phase, so a scrolling container counts): a touch scroll must never open a sheet. The `scroll` listener is armed lazily inside `pointerdown` and removed by `cancel()`/on fire/on unmount, not always-on — `addEventListener` is synchronous, so a listener added the instant the press starts still catches the first scroll frame of a fast flick, at the cost of one active listener per mounted row (~140 on a long list) only while a press is actually down. Nothing is drawn while the timer runs; the feedback is `haptic("medium")` plus whatever the callback opens. The fired press swallows the row's next `click` once through a capture-phase handler on the same element (the `SwipeStrip` precedent), cleared by the following `pointerdown` OR by any `onKeyDownCapture` — a keydown can never be the tail of a pointer-driven press, so it resets the flag rather than risk eating a keyboard Enter/Space activation. A MOUSE `contextmenu` (`pointerType === "mouse"` read off `e.nativeEvent` — React's SyntheticEvent never proxies that non-standard MouseEvent extension onto `e` itself — or `button === 2` where the browser sends no `pointerType`) fires it immediately — desktop right-click opens the same sheet. A `contextmenu` with NO mouse signal is only `preventDefault`ed: that covers Safari's touch `contextmenu` (the press itself already fired or will) and, indistinguishably, a keyboard-invoked menu (Shift+F10 / the Menu key arrive as `button 0` with no `pointerType`). Keyboard-invoked menus on rows are therefore NOT supported — the row's own Enter/Space tap is the affordance. Spread the whole return on the row, `style` included (it kills the iOS callout and text selection) — `onKeyDownCapture` rides along with the same spread. |
| `QuickActions` [C] | What a long press opens — a `CueDialog mode="sheet"` with one full-width, left-aligned `Button variant="ghost" size="lg"` per action and «Cancelar» last. `actions: { label, onSelect?, href?, tone?, icon? }[]`; an `href` action renders `Button href`. Every action closes the sheet. Mounted with `open={open}`, never behind a conditional with a literal `open`. Consumers: `LibraryIndex` (ONE for the whole `/biblioteca` list, driven by `LibraryRow`'s `onQuickActions`), `DayCard` (ONE per card, driven by its `SongRow`s' `onQuickActions` — F3) and `DayCardDisclosure` (per row — there are a handful). |
| `CueStrip` [C] | The navbar cue (R7, spec §12.8) — the member's next service under the title on every route except `/` and `/me` (already carry their own countdown). Fetched client-side after paint (`/api/cue`), like the notification badge, so `Navbar` stays sync and its pages stay static/ISR. A shared in-flight promise dedupes the phone/desktop pair that both mount per page; `sessionStorage` caches `{c, t, id}` keyed by the signed-in member's `sanityId`, 60 s TTL — a stale-identity hit (shared machine, impersonation switch) is a miss, not a leak. Reserves the line's height whenever armed (not just once the label paints), since `Navbar` remounts every route change. |
| `Blackout` (`blackout()`) [C] | The stage vocabulary's exit (spec §12.8), used once: sign-out. A plain CSS opacity transition on a `div` appended to `document.body` — deliberately not `motion`, since it fires from a synchronous click handler and outlives the component that rendered the trigger. Returns `{ done, cancel }`; `cancel()` removes the overlay so a `signOut` that throws never leaves the page permanently black. Idempotent while live (a second call returns the same handle). Instant (no transition) under reduced motion. |
| `PlayPauseGlyph` [C] | The play/pause morph (R4, spec §5.3) — a `Presence variant="scale"` pair of glyphs in one `inline-grid` cell, `{ playing, size? }`. `aria-hidden`; the accessible name stays on the hosting button (`SongAudioSection`, `AudioTransport`, `PracticeCluster`). |
| `Equalizer` [C] | The playing track's tell (R4, spec §5.3) — three `.brand-eq-bar` bars (`brand.css`, beside `.brand-tempo-pill`), `{ playing, className? }`, `data-playing` on the host span. CSS-only: static at `scaleY(0.3)` idle, animating only under `[data-playing="true"]` — the one ambient animation in the app, and it stops with the audio. Rendered always on `SongAudioSection`'s card title row (`ml-auto`) so the row never shifts when it arrives. |
| `PullToRefresh` (+ `pullModel.ts`) [C] | Pull-to-refresh on the phone (R7, spec §12.8), mounted once in `app/(client)/layout.tsx`. THE CONTENT IS NEVER TRANSLATED — the indicator is a sibling `fixed` rail at the viewport top, never a transform on `<main>` (ADR-0031: a transformed ancestor becomes the containing block for every `position: fixed` descendant). Arms only where the tab bar is (`BottomNav`'s `has-bottom-nav` class + a coarse pointer, re-checked via `MutationObserver`) and while no `CueDialog` layer is open. Window `touchstart` is passive and always attached while armed; the `{ passive: false }` `touchmove`/`touchend`/`touchcancel` trio is attached per-gesture, only once `touchstart`'s checks pass, and removed again on end/cancel. `touchstart` clears the travel refs FIRST, before its multi-finger bail — a second finger landing mid-pull (a `touchstart` that already carries two touches) must not leave the first pull's travel sitting for that touch's own lift to commit. `preventDefault()` fires only past 8 px of downward travel at `window.scrollY <= 0` (`<=`, never `===` — the iOS rubber band drives `scrollY` negative). The axis locks once, on the first move past the slop; a horizontal decision hands the touch back for good. `[data-pull-ignore]` on a touch's ancestor opts the surface out (`SwipeStrip`'s drag host; `AvailabilityGrid`'s months while «Seleccionar fechas» is armed). `pullModel.ts` is the neutral arithmetic (`THRESHOLD`, `MAX`, `pullProgress`, `shouldRefresh`, `railHeight`) — the gesture can't be driven with fidelity in jsdom, so the numbers are what's actually tested. A committed pull calls `router.refresh()` once, with a 600 ms floor on the rail's "refreshing" state so a fast, already-warm route doesn't flash and vanish. `brand.css`'s `overscroll-behavior-y: contain` moves to the root element under `html.has-bottom-nav` so Chrome Android's own pull-to-refresh doesn't fire underneath this one. |
...
| `SongPracticeFixture` (`app/(gallery)/theme-gallery/[theme]/[fixture]/fixtures/`) [C] | The `song` gallery fixture — the song hero (pills + transposer drawer), a `ChordChart` and two `SongAudioSection` cards, on a hermetic page. Mounts its own `PlayerProvider` (`usePlayer()` throws without one) and `TransposeProvider nativeKey="G"`; its track URL is an inline data-URL silent WAV, since the route has no network. Since R6 it also renders the lyric block inside the song page's OWN prose wrapper class string, byte-identically, so the fixture shows the real cascade — and a `TutorialPoster`, which is the ONE thing in the gallery that reaches the network (`i.ytimg.com` via `next/image`, hard-coded in the production component); `gallery.spec.ts` stubs that route. This is what dev-verify screenshots for the transposer. |
| `NavFixture` (same directory) [C] | The `nav` gallery fixture (R6 Task 7) — `SectionNav` plus a `BottomNavBar` with its «Más» sheet OPEN from first render (a baseline needs the sheet present, not a button a harness must click before every capture). Hermetic: props only, and it hosts `BottomNavBar`, never `BottomNav`, which reads the session. Mounts `CueDialogProvider` directly, as `DialogFixture` does. The bar is `fixed bottom-0 … lg:hidden`, so it is the phone project's subject; the desktop capture shows the section nav, and the root's `pb-24` keeps the bar off it. |
...
| Component | Purpose |
|-----------|---------|
| `AdminPanel` | The Control Room's one tree: ONE `AdminRail` plus one body `key={tab}`/`animate-fade-in` (a tab change remounts the incoming panel). It owns the tab reducer, `?tab=` and the integrity hook — and NO panel's contents: Miembros left for `MembersPanel` in R5 Task 3, so nothing member-shaped (Fuse index, dialogs, member writes) is built on the five tabs that never show it. No `brand-surface` box around a panel and no `.brand-admin-shell` around the panel — the page is the workspace ([ADR-0035](adr/0035-the-admin-shell-is-gone.md)); `adminShell.test.tsx` is the guard. `?tab=` behaviour is unchanged (`adminTabUrl.test.tsx`, whose queries are scoped to the strip because both nav layouts are in the DOM). Calls `useIntegrityQueue()` once, at the top level, and feeds the queue to `IntegrityQueuePanel` and the tone/count to `AdminRail`. `ActivityPanel`/`ContentPanel`/`AvailabilityPanel`/`ProposalsPanel`/`MembersPanel` load through `next/dynamic({ ssr: false, loading: PanelSkeleton })` (R5 Task 6) — only `ServicesPanel` and `IntegrityQueuePanel` stay eager, since Servicios is the default tab for most roles and the integrity fetch feeds the rail's dot on every tab. Guard: `adminDynamic.test.ts` (source scan — each panel is imported via `next/dynamic` here and nowhere else statically). |
| `MembersPanel` | The Miembros tab: the member list, its ministry/type/role filters and Fuse search, the four member dialogs and every member write (add, edit, password, photo, delete, access). Row actions are ONE `Menu` (R5 ruling 6) behind a `Button variant="icon" size="lg"` ⋯ labelled «Acciones de {alias}»: Editar · Contraseña and, for a `super-admin`, Ver como este miembro · Deshabilitar/Habilitar acceso · Eliminar — nothing depends on hover, so touch reaches every action. Taking access away opens a confirm `CueDialog` first (decision O) and writes through the same `PATCH /api/admin/members/:id/disable`; restoring it does not, and a REFUSED write keeps the confirm open with its own `CueDialogStatus` (`handleDisableAccess` returns `res.ok`). All five dialogs are mounted always and opened by a boolean, with the payload (kind + member) held in state that outlives the close so no body blanks during the exit; the bodies are keyed on an open counter so a reopen starts fresh. Exports `MemberForm`, `MinistryScopeBar` and `resolveMinistryScope` (absent `ministries` means worship — read through `normalizeMinistries`, never raw). Guards: `membersPanelMenu.test.tsx`, `memberListMinistryScope.test.tsx`, `MemberForm.test.tsx`. |
| `AdminRail` | The Control Room's section nav — the ONLY one. Two layouts of one control: a sticky vertical rail at `lg+` (icon + label, `SlidingIndicator id="admin-rail" variant="pill"`) and the horizontal underline strip below it (`id="admin-strip"`, `useActiveIntoView(active, true)` — the URL seeds the tab, so the strip centres its active item on MOUNT as well as on a change), both in the DOM with CSS picking one. The strip's phone scroll-fade is a `mask-image` on the scroller, not a colour overlay — the page is `.brand-atmosphere`, which no flat token can match — and it is applied only while there is more to the right, so it never eats the last tab. `ADMIN_TAB_ICON` is the one glyph-per-tab map. The Servicios item carries the integrity dot (`data-integrity`), and every item carries an explicit `aria-label` because `brand.css` hides the labels while the planner is open. `adminRail.test.tsx` is the guard. |
| `useIntegrityQueue` | The ONLY place the three `/api/admin/service-integrity/*` inventories are fetched. `useIntegrityQueue({ enabled, onResolved, cards })` returns `{ queue, tone, sources, loading, reload, resolve }`; the three domains load independently and a failed, in-flight or DISABLED one reads `unknown`, never `clean`. `enabled: false` (a role with no Servicios tab) fetches nothing and makes `reload` a no-op. Loaded once per mount, plus on entering Servicios; «Recargar» is the manual refresh. `resolve` has a stable identity (it is the panel's focus-effect dependency). `useIntegrityQueue.test.tsx` is the guard — and where the fetch mocks live. |
| `ServicesPanel` | Service-readiness cards plus card-owned delete/copy/publish/setlist/proposal flows. Owns the canonical roles/integrity source bundle and opens `MonthGenerator` for **Editar mes**, card roster edits, and **Nuevo**. The editor replaces the tab with a full-width panel rather than a dialog. Since R5 the cards are a BOARD: a `lg:` horizontal snap track (`snap-x`/`snap-mandatory`/`overflow-x-auto`, 380 px `snap-start` cards) in its own `min-w-0` column, so it is the only horizontal scroller here and the page never widens; the phone keeps the vertical list. Every control is a primitive (`Button`, `Skeleton`, `Collapse`); the month pills are `Button variant="pill"` and stay MULTI-select via `aria-pressed` (a `past` pill dims with `opacity-60`, never a second `border-…`/`text-…` beside the variant's own — that override lost to Tailwind's emission order) — since 2026-09-22 the offered months are the ones with a service PLUS the current month and the next two even when empty (`upcomingMonthPills`, `monthPills.ts`), so an empty upcoming month (a camp weekend) can be opened without generating it first; a closed toolbar gate states its reason as a LINE under the buttons rather than a `title`, which `disabled:pointer-events-none` makes unreachable; the local `Modal` takes an `open` prop, so all five dialogs are mounted always with their payload outliving the close. Guard: `servicesBoard.test.tsx`. |
...
| Component | Purpose |
|-----------|---------|
| `ServiceReadinessCard` | One service card; maps over `CARD_SECTIONS` (the constant *is* the render order). Takes `className` (additive LAYOUT utilities only — the board sizes and snaps the root; colour stays here) and `revealAttrs` (`{...revealProps(i)}`). |
...
| `ServicePrimaryAction` | The single primary-action button — a house `Button` that still emits `data-action-kind`/`-rule`/`-route` and keeps `CARD_STYLE.primaryAction`'s 44 px full-width target. The tone is the VARIANT (danger for an integrity/conflict blocker, secondary for a retry, primary otherwise), never a colour override on `className`. |
| `IntegrityQueuePanel` | The "Integridad de datos" panel. Fetches NOTHING since R5: props in (`queue`, `tone`, `sources`, `reload`, `target`, `onResolved`) from `useIntegrityQueue`, which `AdminPanel` owns. Its `Collapse` follows the tone — open until the inventory is proven clean — and a member can still toggle it. Entries render through `AnimatedList` (R5 Task 5) so a RESOLVED entry fades out and the survivors slide up instead of the row blinking away; `AnimatedList` owns the `<li>`, so each entry is the `tabIndex={-1}` div inside it that the explicit-id reveal still scrolls to and focuses. Each entry keeps its own `Collapse`, and the queue derivation is untouched — it lives in `useIntegrityQueue`. Guards: `useIntegrityQueue.test.tsx`, `adminPanelsPolish.test.tsx`. |
...
**320 test files / 5,526 tests** (284 under `app/` + 27 under `scripts/` + 9 harness unit
tests under `e2e/service-readiness/__tests__/`), counted 2026-09-18 (R6 tip `66cb7192`).
Separately, **13 Playwright specs** are **not** part of `npm test`: 11 under
`e2e/service-readiness/`, which run only against the isolated verification deployment (see
[`VERIFICATION_HARNESS.md`](VERIFICATION_HARNESS.md)), and 2 under `e2e/theme-gallery/`
(R6) — the visual-regression baseline, `npm run test:vr`, opt-in through
`THEME_GALLERY_VR_ENABLED`/`THEME_GALLERY_VR_BASE_URL`, darwin-only and never in CI
(ADR-0014). Its 28 committed PNGs (8.5 MB) live under
`e2e/theme-gallery/__screenshots__/{desktop,phone}/darwin/`; see [`MOTION.md`](MOTION.md). Vitest (`environment: "node"`) covers
