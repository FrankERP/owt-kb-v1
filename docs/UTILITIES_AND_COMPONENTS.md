...
- **`snapshot(dates, notes)`** ([useAvailability.ts](../app/components/availability/useAvailability.ts))
  ...
  `WeekendList` and to `AvailabilityGrid`, which keeps only its paging. The hook's
  ...
  span at 366 days, and no-ops on a reversed range; `MyAvailabilityPanel`'s «Rango…» panel is
  its one caller.
...
| `availability/MyAvailabilityPanel` [C] | **`/me`'s availability host (R3)** — renamed off `AvailabilityPanel` in fix round 1, which shared its name with the unrelated admin `AvailabilityPanel` below (different directory, different job): calls `useAvailability` once and renders `WeekendList`, the shared «Repetir…» recurring panel, «Rango…» (F1, Frank's look: a `Collapse` with two `DateField kind="date"` inputs — «Desde»/«Hasta», `Hasta`'s `min` tracks the chosen `Desde` — calling `applyRange`; «Marcar»/«Quitar rango» stay disabled until both dates are set and `hasta >= desde`), «Guardar» (`busy`/`busyLabel`; success is a `useToast` "Guardado ✓", not an inline flash), the dirty/error/held-conflict notices, the `AvailabilityGrid` behind a «Ver calendario» `Collapse`, and the one `NotePopover` both surfaces open. Opening «Repetir…» or «Rango…» closes the other. «Quitar serie»/«Quitar rango» each call `closeNote()` before dropping their dates, so a popover pinned to one of them cannot outlive it. |
