...
| `TempoPill` [C] | `app/components/song/` — the BPM pill, tapped to beat the song's tempo. CSS-CLOCKED, never a timer (R4 ruling 7): `.brand-tempo-pill::after` in `brand.css` scales and fades once per beat at the pill's own `--tempo-period` (`tempoPeriodMs(bpm)`), so nothing drifts, nothing leaks on unmount, and the global reduced-motion rule collapses it with everything else. `data-active` is absent while idle. Tested — including that the timer count stays at zero. |
