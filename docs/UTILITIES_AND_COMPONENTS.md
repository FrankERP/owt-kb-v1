...
| `CueDialog` [C] | The ONE dialog shell — never a hand-rolled `fixed inset-0` scrim. `mode="modal"` \| `"sheet"` (a sheet is only a sheet below 640px); drag-to-dismiss from the sheet's head (handle + title bar; 150 px of travel or a 0.5 px/ms flick — `SHEET_DISMISS`); `onDismiss(reason)` where `reason` is `"escape" \| "backdrop" \| "drag"`. Traps focus, stacks layers, supports a portalled focus "satellite". Always render `<CueDialog open={x}>`, never `{x && <CueDialog open>}` — see `cueDialogMount.test.ts`. |
