...
<<<<<<< HEAD
| `DayCardDisclosure` [C] | A non-next service on `/` or `/me`, collapsed to one line (day · date · countdown) that opens a `Collapse` onto its `DayCard`. **R7:** a long press opens its `QuickActions` — «Ver en calendario» (`router.push("/schedule")`) and «Añadir a mi calendario» (this ONE service as an `.ics` through `buildICS`, the same blob download `AddToCalendarButton` does for the member's whole list). The UID is `serviceId ?? roleId ?? `${date}-${day}`` (`normalizeText`-folded to strip accents/case) — `serviceId` is the service document's own `_id`, set at both `/` and `/me` call sites, so a weekend service gets the SAME UID `/me`'s own export uses (`<_id>@owt`) rather than one keyed off its day/date string. |
=======
