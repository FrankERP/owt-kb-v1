...
- **`createMetronome({ bpm, beatsPerBar, onStop?, audioContext? })`, `CLICK`**
  ([metronome.ts](../app/components/song/metronome.ts)) — the click under the tempo ring (R4
  ruling 11). Browser-only but React-free and `motion`-free; `TempoPill` is its only importer.
  A "tale of two clocks" LOOKAHEAD SCHEDULER: a self-rescheduling `setTimeout` every
  `CLICK.lookaheadMs` (25 ms) books every beat falling inside the next `CLICK.scheduleAheadMs`
  (100 ms) on `ctx.currentTime`, each one an `OscillatorNode` (sine; `accentHz` 1000 on beat 1
  of the bar, `beatHz` 800 otherwise) through a `GainNode` ramped to silence over
  `CLICK.lengthMs`. Never a `setInterval` — a stop must never race a queued tick. The
  `AudioContext` is created LAZILY on the first `start()`, which therefore has to be called from
  a user gesture, and is reused across starts; `stop()` clears the loop and SUSPENDS the context,
  never closes it (a closed context cannot be resumed). Where no `AudioContext` exists (prefixed
  or not) `start()` is a silent no-op and the pill still rings. A `visibilitychange` to hidden
  stops and reports through `onStop`. Tested in `app/components/song/__tests__/metronome.test.ts`.
