...
| `LyricsAutoscroll` [C] | `app/components/song/` — the «Autoscroll» / «Detener» pill on the Letra section header (R4 ruling 6): hands-free lyrics at the song's own tempo. Drives the WINDOW (`window.scrollTo(0, y)` per frame), never a transform on the section — a transformed ancestor becomes the containing block for every `position: fixed` descendant (ADR-0031), and the audio transport, toasts and song FAB all live on this route. `requestAnimationFrame` only, never `setInterval`: `dt` is the gap between two real frame timestamps, clamped to 50 ms so a backgrounded tab cannot leap. Speed is `autoscrollPxPerSecond(el.offsetHeight, lines, bpm)` measured once at start (`bpm: null` → `DEFAULT_BPM`); `lines` is the ChordPro chart's `countLyricLines` or, for PortableText, one block ≈ one line (an approximation the clamped speed absorbs). Stops itself when the section's `bottom <= window.innerHeight`. A `touchstart` / non-mouse `pointerdown` pauses; `touchend`, `touchcancel`, `pointerup` and `pointercancel` all lift it (a promoted drag fires `touchcancel`, never `touchend` — miss it and the pill sits on «Detener» for good). A lift that actually scrolled waits for `scrollend` rather than resuming into iOS momentum, arming a 400 ms (`SCROLLEND_FALLBACK_MS`) `setTimeout` backstop that resumes on its own if `scrollend` never arrives — cleared on `scrollend`, toggle-off and unmount; a lift that scrolled nothing resumes at once. Resume re-seeds `y` from `window.scrollY` and keeps re-seeding for the next 3 frames, but never every frame — at low speeds a sub-pixel advance rounds back to the same scroll offset and the loop would stall. A `wheel` ends the run outright, because a mouse has no lift event to resume on and a control reading «Detener» over a still page lies (scrollbar and keyboard scrolls are not intercepted — phone-first by design). The pill carries `min-h-[44px]`. Every listener is `{ passive: true }` and every frame is cancelled on toggle-off and on unmount. Tested. |
...
| `NAVBAR_H_CLASS` (`app/utils/navbarHeight.ts`) [N] | The ONE spelling of the top bar's height (`"h-20 lg:h-24"`) — `Navbar` and `Skeleton.tsx`'s `NavbarSkeleton` both import it so a `loading.tsx` placeholder can never drift from the real navbar's height; `navbarHeightSync.test.ts` is the guard (see [MOTION.md](MOTION.md)). |
