...
| `DayStrip` [C] | **The `/schedule` WEEK strip (R2)** — seven cells (`grid-cols-7`), every day of the visible week rendered but only service days pressable; opens on today's week (or the anchor month's first day's week, `mondayOf(anchorMonth + "-01")`, when today falls outside that month). A `SwipeStrip` drag pages the week client-side (`onWeekChange` scrolls the agenda to it); the header's arrows page the month instead — two axes, two gestures. Today's dot pulses once on reveal. Cells share the month grid's tone classes so the two views can't disagree about what a colour means. |
