...
- **`nextWeekends(todayIso, count)`**, **`weekendLabel(weekend)`** ([weekends.ts](../app/utils/weekends.ts))
  ...
  `new Date(iso)`). Each `Weekend` spans Friday (the rehearsal day) through Sunday (F1, Frank's
  look: "we have rehearsals those days too"). `nextWeekends` counts the CURRENT weekend while its
  Sunday is still ahead, so a member opening `/me` on the Saturday or Sunday they are living
  through still sees that row first rather than the one after it. `weekendLabel` renders one
  line, Friday through Sunday — «11 – 13 sep», or «30 oct – 1 nov» when the span crosses a month.
...
- **`snapshot(dates, notes)`** ([useAvailability.ts](../app/components/availability/useAvailability.ts))
  ...
  `WeekendList` and to `AvailabilityGrid`, which keeps only its paging and its drag-select
  gesture (F2). The hook's
...
| `availability/MyAvailabilityPanel` [C] | **`/me`'s availability host (R3)** — renamed off `AvailabilityPanel` in fix round 1, which shared its name with the unrelated admin `AvailabilityPanel` below (different directory, different job): calls `useAvailability` once and renders `WeekendList`, the shared «Repetir…» recurring panel, «Guardar» (`busy`/`busyLabel`; success is a `useToast` "Guardado ✓", not an inline flash), the dirty/error/held-conflict notices, the `AvailabilityGrid` behind a «Ver calendario» `Collapse`, and the one `NotePopover` both surfaces open. **F2 retired the panel's own «Rango…» button and `Collapse`** — those Desde/Hasta fields moved into `AvailabilityGrid` beside the drag gesture that expresses the same range. `openNote(iso, anchor, isos?)` now forwards an optional `isos` (the whole dragged range) into the `NoteAnchor`. «Quitar serie» calls `closeNote()` before dropping its dates, so a popover pinned to one of them cannot outlive it. |
| `WeekendList` [C] | **`/me`'s default availability surface (R3; Fridays added F1)** — the next ten weekends from `nextWeekends(todayIso, 10)`, each an «11 – 13 sep» row spanning Friday (rehearsal day) through Sunday, with `VIE`/`SÁB`/`DOM` `Button variant="pill" tone="availability" size="lg"` toggles (`aria-pressed` = «no puedo»; the 44 px touch target also applies to the «Razón» ghost) and a service dot on a day in `serviceDates` (Fridays rarely carry one — rehearsals aren't service documents, but a special Friday service still would). A day already in the past renders `disabled` — it is not a toggle. Toggling fires `haptic("selection")`. |
| `AvailabilityGrid` [C] | **The twelve-month grid (R3; drag-select F2)**, moved to `app/components/availability/AvailabilityGrid.tsx` in fix round 1 (was `app/components/AvailabilityCalendar.tsx`) — a controlled view (`state`, `serviceDates`, `openNote`, `closeNote`, `noteIso`) behind `/me`'s «Ver calendario» disclosure, for what ten weekend rows cannot say. Owns paging AND the F2 drag-select gesture: a «Seleccionar fechas» ⇄ «Listo» `Button` (never a long-press — that gesture is already the phone browser's text-selection/callout trigger) arms pointer-capture drag over the months container (`touch-action: none`, `user-select`/`-webkit-user-select: none`, `-webkit-touch-callout: none`, `onContextMenu` cancelled while armed); `isoFromPoint` (`dragSelect.ts`) resolves the day under a captured pointer via `elementFromPoint`; one pointer (`isPrimary`/`pointerId`) owns the gesture and a `dragRef` mirror lets `pointerup` read the live span rather than a possibly-stale render closure. The next month fades in as an absolutely-positioned OVERLAY inside the last visible tile's own bounds (`shadowOpacity` over a 120px reach from the tile's bottom) — never a fourth tile that would grow the page — `pointer-events-none`/`aria-hidden` while fading, and solidity LATCHES for the rest of the drag once the finger first reaches the tile's bottom edge (the overlay's own cells sit inside the host's padding/border and never reach that edge themselves). Release calls `applyRange`, flips the page if the end landed in the shadow month, and opens the range's `NotePopover` from a post-commit effect anchored to the first still-mounted day (`scrollIntoView({ block: "nearest" })`). The Desde/Hasta `DateField`s under «Rango por fechas» are the keyboard/VoiceOver path to the same range. |
