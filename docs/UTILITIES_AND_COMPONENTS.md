...
| Component | Purpose |
|-----------|---------|
| `AdminPanel` | The Control Room's one tree: ONE `AdminRail` plus one body `key={tab}`/`animate-fade-in` (a tab change remounts the incoming panel), and member management inline (Fuse.js search, add/edit modal, role-gated). No `brand-surface` box around a panel and no `.brand-admin-shell` around the panel — the page is the workspace ([ADR-0037](adr/0037-the-admin-shell-is-gone.md)); `adminShell.test.tsx` is the guard. `?tab=` behaviour is unchanged (`adminTabUrl.test.tsx`, whose queries are scoped to the strip because both nav layouts are in the DOM). Calls `useIntegrityQueue()` once, at the top level, and feeds the queue to `IntegrityQueuePanel` and the tone/count to `AdminRail`. |
