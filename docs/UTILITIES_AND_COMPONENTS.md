...
| `NavMenu` [C] | Nav menu + notification badge (the badge pops in on `Presence appear variant="scale"`). |
| `BottomNav` [C] | Mobile bottom tab bar — five worship tabs or the kids-only set; «Más» is a `CueDialog` sheet. Publishes its measured height as `--bottom-nav-h` + a `has-bottom-nav` class on `<html>` while on screen; hidden ≥ `lg` and on `/auth*`/`/studio*`. |
