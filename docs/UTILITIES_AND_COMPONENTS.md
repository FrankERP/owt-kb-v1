...
| `SectionNav` [C] | In-page section anchors, plus — since R4 — the song page's practice cluster. One `IntersectionObserver` on `#song-hero` (threshold 0) flips `heroGone`, inset by a `rootMargin` of `-<bar bottom>px 0px 0px 0px` measured from the bar's own `getBoundingClientRect().bottom` at observer creation (the bar is sticky directly under the navbar, so its bottom edge measures both) — otherwise the hand-off waits for the viewport's top edge and the hero's controls are already hidden behind the chrome while the bar still shows nothing, and the optional `practice` prop then fades in through a `Presence variant="fade"`. The right-hand slot (`ml-auto … shrink-0 … min-h-[44px]`) is ALWAYS rendered, empty or not, so the bar's height never changes when the cluster arrives; the links keep their own scroll box and scroll behind it on a phone. |
