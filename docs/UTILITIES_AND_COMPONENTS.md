...
| Component | Purpose |
|-----------|---------|
| `SongSheet` [C] | Full lyrics + chords overlay (PortableText, focus-trapped, play-history) — driven by `PlayerContext`. Its meta row's BPM is a real `TempoPill` (`size="sm"`, `enabled={isOpen}`) when `bpm` parses to a positive number, and the old static span when it does not ("libre", "70-80"). |
