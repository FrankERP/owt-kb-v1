...
| `LibraryRow` [C] | One song row: key · title/artist · BPM · tags. A plain `<button>` in an `<li>`, not the `Button` primitive — the recorded row exemption (`DayCard`'s setlist rows precedent): the row IS the affordance. Memoized (~140 rows). **R7:** a long press (`useLongPress`) opens its `QuickActions` — «Abrir» (the same `openSheet` the tap calls) and «Copiar enlace» (`/posts/<slug>` to the clipboard, confirmed or refused by a `useToast` toast); the tap itself is unchanged and the press swallows its own click. |
