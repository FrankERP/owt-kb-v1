...
- **Solve 0 fixes the count first.** Order: solve 0 minimises the violation count alone, with
  ...
  If solve 0 finds nothing, no stage carries a ceiling and `violation_ceiling_proven` is `false`.
...
- **Refusals** (all `ValueError` → `ok: false`): a malformed entry, more than 100 entries (never
  ...
  Saturday, two different pins for one person in one service. Exact duplicates collapse.
