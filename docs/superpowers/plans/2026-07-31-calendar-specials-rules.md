...
7. Wire into `handleAuto` so it runs for special columns **even when the weekend solve fails**. There are **three** exits to cover, not one: the pre-flight refusal (`:1206-1211`), the `!res.ok || !response.ok` return inside `try` (`:1225-1228`), and the gate that stops `handleAuto` being called at all (`autoState.disabledReason = gateBlocked`, `:1375`) — a month with no Sunday leads must still fill its specials. **Each exit needs all three setters, not just `setUnfilled`:** the success path does `setCells` → `setUnfilled` → `setDrafts` (`:1239-1248`), and `handleAuto` writes cells directly rather than routing through `handleCellsChange`. Miss `setDrafts` and a filled special renders populated in the grid while posting an **empty** document. Label it in the UI as a distinct mechanism from the solver; it approximates fairness greedily and does not enforce caps (non-goals).
...
- **In `rankCandidates`, a `slack 1` member's rendered `load` is unchanged and her position moves only as a tie-break** (P7b). Nothing else in the plan can catch a `load + N` applied to the field.
