...
**Date:** 2026-08-08
**Status:** **SUPERSEDED IN PART, 2026-08-11.** This document defines **18** roles. Child B
shipped **30** base roles and 23 composed tokens: it added five for values this vocabulary
named but did not define (two "deep" state surfaces whose values were folded into one slot
as "· deep", and three overlay navies it declined to collapse), and seven more for its own
"Literals with NO role here" table (`--elevation`, six `--chart-*`). Child B also corrected
two statements here — the `icons.tsx` prescription, which named a form that is silently
dropped inside an SVG attribute, and the owner of the four `white` SVG attributes.
**For current state read [`…-B-token-migration.md`](2026-08-08-light-mode-B-token-migration.md).**
This remains the record of the original vocabulary and the reasoning behind it.
