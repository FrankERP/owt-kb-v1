...
| Child A, Phase C (the cutover) | **Implemented on `feat/child-a-thread`.** Both message routes, the transition's append, §2's two-part rule, the reads and `ProposalThread` on both surfaces. Gates green (tsc, 4257 tests, eslint 0 errors). Four review rounds; each found a defect the previous fix introduced, all in the post-commit read path, and the last entry is a verification |
| Child A, Phase D (the migration) | **`--apply` HAS RUN — 2026-08-26, with Frank's consent.** 8 documents / 10 messages / 0 failures. Verified against the count the legacy fields predict, not a hard-coded one. **One-shot: never run it again.** Steps 6–9 remain |
