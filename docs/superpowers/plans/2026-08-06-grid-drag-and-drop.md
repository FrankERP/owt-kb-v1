...
Pointer drag at desktop width, drop targets at cell granularity, refusals surfaced inline, prompt only on C4. **The C4 confirm is asynchronous:** a pending proposal held in state must be applied against the `cells` snapshot captured with it, or re-gated on confirm — otherwise the force lands on a `cells` that moved under it while the prompt was open. State explicitly whether this reuses `SetlistEditor.tsx:335-339`'s HTML5 drag idiom or introduces pointer events, and why.
