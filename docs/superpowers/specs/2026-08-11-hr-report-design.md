...
2. **`hr-report` skill** — canonical `~/.agents/skills/hr-report/SKILL.md` +
   ...
   d. Render one self-contained, theme-aware HTML page (inline SVG charts — load
      the `dataviz` skill first; no external assets): dispatches-per-agent bar,
      outcome-mix donut, cycle timeline, token/time charts when data exists, award
      cards, PIP section, HR's retrospective paragraph.
   e. Publish as artifact; write the identical file to
      `docs/agents/reports/<YYYY-MM-DD>-<period>.html`; commit; log the dispatch
      (with tokens/duration — the new fields' first use).
...
3. **hr-officer report mode** — short charter addition (both platforms): when
   ...
   verbatim (never recompute), and return: STAFF REVIEW (per-agent, one line),
   AWARDS — headline *Agent of the {Week|Month|Quarter}* scaled to period, plus at
...
- Missing token/duration data → shown as "not recorded before 2026-08-11", never
  interpolated.
- Malformed JSONL lines → skipped with a visible count in the report, never repaired.
