...
1. `special_role` is in `ALLOWED_TYPES` on the create route (`app/api/admin/roles/route.ts:119`) and first-class in the read model (`app/utils/serviceReadModel.ts:10`, `:53`, `:74`).
...
6. `type ServiceType = "sunday_role" | "saturday_role"` (`MonthGenerator.tsx:32`), so `GridColumn`, `DraftCard` and the `preflight` signature all *refuse* specials today. **Widening it is NOT broadly `tsc`-visible.** Only `Record<ServiceType, …>` (fact 27) breaks the build; every other weekend/special distinction is an `===` comparison that keeps compiling and silently takes the Saturday path. Four such branches exist and each is wrong for a special:
   ...
   - `isSolvable` (`plannerModel.ts:215-219`): true for lead/bgv on any non-Saturday column → special columns become **solvable**, contradicting E4/E5.
...
10. `GridCell` (`plannerModel.ts:48-53`), `cellKey(date, rowId)`, `assignedForDate`, `categoryDuplicatesForDate` (`PlannerGrid.tsx:111-127`, `:137-145`, `:276`), `cellsByDate` (`plannerModel.ts:600`, `:776`) and `typeOf = columns.find(c => c.date === d)` (`MonthGenerator.tsx:1166`) all assume **one column per date**. Two specials on one date would share one roster.
...
6. `draftToDayCardProps` must not label a special "Domingo" (fact 26), and `PlannerGrid.tsx:539` must not label its column "Sábado" (fact 6).
7. The four silent `===` branches of fact 6: `rowAppliesTo`, `isSolvable`, `weekForColumn`/`applySolveResponse`, and the column label. Consider making the widened type load-bearing — e.g. a `Record<ColumnType, …>` for the label and for row applicability — so the compiler earns the claim fact 6 could not.
8. `buildColumns` (`plannerModel.ts:226-236`) takes only `sundayDates`/`activeSatDates`; specials need a third input.
9. A client-side guard refusing confirm on an empty special name — `canonicalizeCreatePayload` 400s (`roleCreationReceipt.ts:135-136`) and `handleConfirm` has none.
10. `handleColumnSwap` (`MonthGenerator.tsx:1156-1188`) would let two special columns swap rosters while each `service_name` stays with its column. Decide and state whether that is intended; the weekend swap already refuses to cross service types.
