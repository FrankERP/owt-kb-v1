...
not prove its relaxation set was minimal (its violation-only solve stopped short of optimality —
the solver spec §4 is canonical, and **an absent field on a pinless request is not `false`**), so the notices may list more rules than the pins
