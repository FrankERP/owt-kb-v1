...
- [ADR-0036: The publish transition's setlist notice is not debounced](0036-the-publish-notice-is-not-debounced.md) — why «Setlist listo» took 5–10 minutes to reach the team, why retuning the debounce globally and raising the Scheduler to `*/1` were both rejected, and what publish-then-edit costs now
- [ADR-0037: The lexicographic objective's weights use per-tier maxima](0037-objective-weights-use-per-tier-maxima.md) — why the solver's eight-tier weight ladder overflowed int64 on ordinary months, why ortools' `MODEL_INVALID` was indistinguishable from an infeasible fairness tier and so lost fairness optimisation in silence, and why months with history degrade-and-report instead of failing
- [ADR-0038: A «Noche de alabanza» is a special's `format`, not a fourth role type](0038-worship-night-is-a-special-format.md) — why per-song leaders are a `special_role.format` flag rather than a fourth document type, and the release-window hazard of running the old writers against the new data mid-rollout
