...
- [ADR-0027: Agent verification on dev uses a local read-only runner, not a server login path](0027-agent-dev-verification-is-a-local-read-only-runner.md) — why `scripts/dev-verify.ts` signs in as a dedicated retired member instead of minting a server verification session, and the `loginEvent`/`lastSeen` consequences accepted along the way
