...
| [0010](0010-specials-fill-locally-not-in-the-solver.md) | Fill special services locally; move the rules to Sanity | Accepted, amended by [0041](0041-the-fairness-history-is-derived-from-stored-services.md) — P6 implemented 2026-08-03 |
...
- [ADR-0032: Layer 1 of the notification outbox runs on Google Cloud Scheduler](0032-layer-1-runs-on-cloud-scheduler.md) — why the five-minute flush moved off GitHub's starved `schedule`, why not Vercel Pro, and the third home `CRON_SECRET` now has
- [ADR-0033: Retire the theme announcement rather than persist its dismissal](0033-the-theme-announcement-is-retired.md) — why a `localStorage` dismissal kept re-showing the banner to members who had dismissed it, and why neither a cookie nor a Sanity flag was worth spending on a month-old notice
- [ADR-0034: A dialog effect that moves focus depends on edges only](0034-dialog-focus-effects-depend-on-edges-only.md) — why `CueDialog`'s focus effects deliberately omit `onDismiss` and two ref props, the three typing surfaces that broke when they did not, and why `useCallback` at the call sites was rejected as the fix
- [ADR-0035: The Control Room is the page — the admin shell is gone](0035-the-admin-shell-is-gone.md) — why `/admin`'s bordered shell, its per-tab panel boxes and its six copies of the tab bar were deleted rather than repaired in CSS, and why the planner's full-screen portal and `MonthGenerator`'s hand-centring stay regardless
- [ADR-0036: A «Noche de alabanza» is a special's `format`, not a fourth role type](0036-worship-night-is-a-special-format.md) — why per-song leaders are a `special_role.format` flag rather than a fourth document type, and the release-window hazard of running the old writers against the new data mid-rollout
- [ADR-0037: The publish transition's setlist notice is not debounced](0037-the-publish-notice-is-not-debounced.md) — why «Setlist listo» took 5–10 minutes to reach the team, why retuning the debounce globally and raising the Scheduler to `*/1` were both rejected, and what publish-then-edit costs now
- [ADR-0038: The lexicographic objective's weights use per-tier maxima](0038-objective-weights-use-per-tier-maxima.md) — why the solver's eight-tier weight ladder outgrew CP-SAT's integer-objective ceiling (INT64_MAX / 2) on ordinary months, why ortools' `MODEL_INVALID` was indistinguishable from an infeasible fairness tier and so lost fairness optimisation in silence, and why months with history degrade-and-report instead of failing — giving up the floating-point objective `main` fell into on some of them (trade-off accepted 2026-09-23)
- [ADR-0039: MCP client registration is stateless DCR](0039-mcp-client-registration-is-stateless-dcr.md) — why the client id is a signed token and nothing is written to Sanity at registration, why CIMD was not advertised despite MCP 2026-07-28 preferring it, and the `jose@6`/next-auth `jose@4` nesting the pin causes
- [ADR-0040: MCP reads mirror the readiness loader and the publish check instead of widening them](0040-mcp-reads-mirror-the-readiness-loader-and-publish-check.md) — why `loadServiceSnapshot` and `publishRefusalFor` are pinned copies of writer-owned logic rather than widenings of it, the three tests that pin them, the one place the copy deliberately sees more (`setlistDrafts`, issue #97), their exits (D2 at P3, D1 only by a critical-tier widening) and why neither may be consolidated in a routine cleanup. Released with P1 2026-09-25 (PR #98, `main` `a04edb43`)
- [ADR-0041: Pins are fixed variables, and the rules they contradict go soft under a proven ceiling](0041-pins-are-fixed-variables-and-rules-go-soft-under-a-ceiling.md) — why a pinned seat is a fixed variable with scoped candidacy rather than a removed seat, why the hard spreads give pin slack keyed on the SERVICE (not the role, and not by subtraction), why every contradictable rule goes soft per instance instead of an enumerated exemption list, and how solving for the violation ceiling first reconciles the soft pair rule with ADR-0010: the number of rules set aside is never increased for fairness. Seven rejected designs, each killed by execution
- [ADR-0041: The solver's fairness history is derived from stored role documents, not `localStorage`](0041-the-fairness-history-is-derived-from-stored-services.md) — **amends ADR-0010**; why the history's per-browser gap is closed by a server-side derivation over canonical weekend role documents rather than a stored history document, the staged dormant-then-cutover switch (`SOLVER_HISTORY_SOURCE`), the diff tool's third `unverified` verdict for the H3 narrowing, and the known limitation that the history still only feeds a solver objective ADR-0038 shows is skipped on most real months (issue #94, open). Implemented on `claude/mcp-p2-solver-history`, Delivery 1 — not yet released
