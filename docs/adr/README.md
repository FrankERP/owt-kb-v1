...
- [ADR-0035: The lexicographic objective's weights use per-tier maxima](0035-objective-weights-use-per-tier-maxima.md) — why the solver's eight-tier weight ladder overflowed int64 on ordinary months, why ortools' `MODEL_INVALID` was indistinguishable from an infeasible fairness tier and so lost fairness optimisation in silence, and why months with history degrade-and-report instead of failing
...
- [ADR-0037: The Control Room is the page — the admin shell is gone](0037-the-admin-shell-is-gone.md) — why `/admin`'s bordered shell, its per-tab panel boxes and its six copies of the tab bar were deleted rather than repaired in CSS, and why the planner's full-screen portal and `MonthGenerator`'s hand-centring stay regardless
