...
- [ADR-0038: The lexicographic objective's weights use per-tier maxima](0038-objective-weights-use-per-tier-maxima.md) — why the solver's eight-tier weight ladder overflowed int64 on ordinary months, why ortools' `MODEL_INVALID` was indistinguishable from an infeasible fairness tier and so lost fairness optimisation in silence, and why months with history degrade-and-report instead of failing
