...
describe("brand.css — (b) theme parity, dormant until .light carries custom properties", () => {
  ...
  it("counts both sides, so a role added to one block alone cannot pass", () => {
    // 97 colour properties per side — 67 base roles and 30 composed tokens. The four
    ...
    // 95 when `--skeleton-base` and `--skeleton-sweep` (Task 6 fix round 1,
    // 2026-09-08) replaced the two raw alpha literals `.brand-skeleton`'s rule
    // body had used, 96 when `--sheen-highlight` (Task 7, 2026-09-08, same
    // ruling) replaced the one raw alpha literal `.brand-btn-sheen::after`'s rule
    // body would otherwise have used, and 97 when `--lit-beam` (Task 8, 2026-09-10,
    // same ruling) replaced the one raw alpha literal `.brand-lit-card[data-lit]
    // ::after`'s conic-gradient stop would otherwise have used — this is
    // registration, not a bumped pin: the tokenLayer.test.ts rule-body-literal
    // pin stays at 69. That edit is the point, not an inconvenience.
