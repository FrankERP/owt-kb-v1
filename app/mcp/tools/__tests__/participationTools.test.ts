...
describe("get_participation", () => {
  ...
  it("reports each included service's publication state, drafts included, and a special's name and time", async () => {
    const result = await getParticipationResult({});
    const payload = result.structuredContent as { services: { serviceId: string; published: string; name?: string | null; time?: string | null }[] };
