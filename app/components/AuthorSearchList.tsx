        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="relative">
            <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar..."
              className="font-label pl-3 pr-8 py-1.5 rounded-lg border border-surface-accent-20 bg-transparent focus:outline-none focus:border-accent text-sm placeholder:text-gray-600 transition-colors w-36 sm:w-48" />
            {query && (
              <button
                type="button"
                onClick={() => setQuery("")}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {grid.map((a) => (
            <Link key={a._id} href={`/author/${a.slug.current}`}>
              <div className="relative group overflow-hidden rounded-xl border border-surface-accent-faint p-4 hover:border-accent-deep/50 dark:hover:border-accent/40 hover:shadow-lg hover:shadow-accent/10 transition-all duration-200 cursor-pointer">
