---
name: improve-owt
description: Make exactly one self-contained, verified, high-value improvement to the Oasis Worship Team app. Use for "$improve-owt", "improve OWT", continuous improvement, or a scheduled OWT improvement iteration. Prefer an empty run over manufactured churn.
---

You are continuously improving the **Oasis Worship Team** app (owt-kb-v1). This
skill performs **exactly ONE** self-contained, verified improvement per run.
Invoke `$improve-owt` for a single iteration. For recurring runs, create a Codex
scheduled task that invokes `$improve-owt`; each run must still perform only one
iteration. Everything needed to understand the project is at the bottom of this
...
6. **Commit + push.** Conventional-commit message, imperative mood, explains the
   *why*. **No AI/Claude attribution or Co-Authored-By trailer.** Then push.
...
- **Spawn an `explorer` or fresh read-only subagent** to hunt concrete bugs
...
- **Ask Codex to review the working diff**, or spawn a fresh `skeptical-reviewer`
...
- **One iteration:** invoke `$improve-owt` in the OWT repository.
- **Continuous in the current task:** ask Codex to repeat `$improve-owt`, using a
  fresh task context per iteration and stopping after an empty run or on request.
- **Durable cadence:** create a Codex scheduled task whose prompt invokes
  `$improve-owt`; choose the cadence explicitly.
- **Wind down:** stop the scheduled task or let an empty run pause the workflow.
